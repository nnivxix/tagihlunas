...
<template>
  ...
  <AppBar titleapp="Add User">
    ...
    <form class="flex flex-col" @submit.prevent="HandleAddUser" action="/users" method="POST">
      ...
      class="bg-light-lemon p-2 rounded-lg" placeholder="john123"
      ...
      <button class="bg-lemon p-3 mt-5 font-bold text-dark rounded-lg" :disabled="!isValid"
      :class="{'bg-gray-300': !isValid, 'text-gray-500': !isValid }">{{ textButton }}</button>
    </form>
  </div>
</template>
