<template>
  <div class="mx-6">
    ...
    <form class="flex flex-col" @submit.prevent="handleLogin">
      ...
      <input type="name" name="name" id="name" v-model="formRegist.name"
        class="bg-light-lemon p-3 mb-5 rounded-lg" placeholder="your name">
      <label for="email" class="text-xl text-dark">email</label>
      <input type="email" name="email" id="email" v-model="formRegist.email"
      ...
      <div class="flex" >
        <input :type="hidePswd ? 'password' : 'text'" name="password" id="password" v-model="formRegist.password"
      ...
      <button title="For temporally, the registration of a new account has not been opened." disabled class="bg-gray-400 p-3 font-bold text-white rounded-lg">Register</button>
    </form>
    <div class="text-center mt-5">
    <p>Already have an account? <router-link class="underline" to="/login"> please login</router-link></p>
...
const handleLogin = async () => {
  try {
    await useAuthUser().userRegister({
